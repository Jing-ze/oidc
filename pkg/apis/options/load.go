...
func Load(jsonBuffer []byte, flagSet *pflag.FlagSet, into interface{}) error {
	v := viper.New()
	v.SetConfigType("json") // Config is in toml format
	v.SetEnvPrefix("OAUTH2_PROXY")
	v.AutomaticEnv()
	v.SetTypeByDefaultValue(true)

	err := v.ReadConfig(bytes.NewReader(jsonBuffer))
	if err != nil {
